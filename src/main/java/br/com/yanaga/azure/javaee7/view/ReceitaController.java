package br.com.yanaga.azure.javaee7.view;

import br.com.yanaga.azure.javaee7.model.Receita;
import br.com.yanaga.azure.javaee7.model.ReceitaRepository;

import javax.faces.bean.ManagedBean;
import javax.faces.bean.SessionScoped;
import javax.inject.Inject;
import java.io.Serializable;
import java.util.List;

@ManagedBean
@SessionScoped
public class ReceitaController implements Serializable {

	@Inject
	private ReceitaRepository receitaRepository;

	private Receita receita = new Receita();

	private String consulta;

	public List<Receita> find() {
		if (consulta == null) {
			return receitaRepository.findAll();
		}
		else {
			return receitaRepository.findByNomeLike(consulta);
		}
	}

	public String novo() {
		this.receita = new Receita();
		return "cadastro";
	}

	public String editar(Receita receita) {
		System.out.println("editar" + receita.getId());
		this.receita = receita;
		return "cadastro";
	}

	public String salvar() {
		receitaRepository.save(receita);
		return "lista";
	}

	public void excluir(Receita receita) {
		receitaRepository.delete(receita);
	}

	public Receita getReceita() {
		return receita;
	}

	public String getConsulta() {
		return consulta;
	}

	public void setConsulta(String consulta) {
		this.consulta = consulta;
	}
}
